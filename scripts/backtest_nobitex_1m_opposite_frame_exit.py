...
DEFAULT_DATA_DIR = "reports/nobitex_intraday_native/1m"
DEFAULT_FEE = 0.0013
...
def direction_series(df: pd.DataFrame) -> np.ndarray:
    ...
    # Same entry logic as the 1H test: frame direction, no ambiguity filter.
...
def main() -> None:
    ...
    print("=" * 118)
    print("NOBITEX NON-IRT | 1M SEQUENTIAL TRADING | OPPOSITE-FRAME EXIT")
    print("=" * 118)
    ...
    print(f"Fee             : {DEFAULT_FEE * 100:.2f}% per side ({DEFAULT_FEE * 200:.2f}% round trip)")
    ...
    for mode in ("consecutive", "discrete"):
        ...
        total_initial = 1000.0 * len(symbols)

        for k in (1, 2, 3):
            per_symbol = [
                simulate_symbol(loaded[s], DEFAULT_FEE, k, mode, 1000.0)
                for s in symbols
            ]
    ...
    output = root / "backtest_opposite_frame_exit_1m.csv"

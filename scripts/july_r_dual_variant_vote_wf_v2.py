...
def evaluate_rules(test: list[list[Candle]], orientations: dict[str, int]) -> dict[str, dict[str, int]]:
    stats = {
        r: {"native_active": 0, "native_correct": 0, "inverse_correct": 0,
            "selected_active": 0, "selected_correct": 0,
            "selected_long": 0, "selected_short": 0}
        for r in RULES
    }
    for candles in test:
        for i in range(len(candles) - 1):
...
def vote_dataset(candles: list[Candle], orientations: dict[str, int], min_active: int) -> tuple[list[dict], dict[str, int]]:
    rows: list[dict] = []
    ...
    for i in range(len(candles) - 1):
        ...
        rows.append({
            ...
            **{f"{r}_decision": "LONG" if s == 1 else "SHORT" if s == -1 else "HOLD" for r, s in per_rule.items()},
        })
    return rows, {
        "long_n": decision["LONG"][0], "long_correct": decision["LONG"][1],
        "short_n": decision["SHORT"][0], "short_correct": decision["SHORT"][1],
        "hold_n": decision["HOLD"][0],
        "all_n": decision["ALL"][0], "all_correct": decision["ALL"][1],
    }


def main() -> None:
    ap = argparse.ArgumentParser(description="Walk-forward dual-variant R voting: every rule has Native and Inverse candidates; one orientation is frozen from training, all selected rule votes are scored equally on test.")
    ...
    rule_rows: list[dict] = []
    print(f"R_DUAL_VARIANT_VOTE_WF train={args.train_month} test={args.test_month} assets={len(symbols)} tfs={','.join(map(str,tfs))}")
    print("Each R has NATIVE and INVERSE. Training freezes the better orientation; Test scores only that selected variant equally in the vote.")
    ...
    for tf in tfs:
        ...
        for r in RULES:
            ...
            st = stats[r]
            print(f"{r:<4} {nacc:7.2f} {iacc:7.2f} {selected_name:^6} {n:5} {nacc if o == 1 else iacc:16.2f} {st['selected_active']:10} {pct(st['selected_correct'], st['selected_active']):7.2f} {st['selected_long']:4} {st['selected_short']:5}")
            rule_rows.append({
                "timeframe_min": tf, "rule": r,
                ...
                "test_selected_acc_pct": round(pct(st["selected_correct"], st["selected_active"]), 6),
                "test_native_acc_pct": round(pct(st["native_correct"], st["native_active"]), 6),
                "test_inverse_acc_pct": round(pct(st["inverse_correct"], st["native_active"]), 6),
                "test_long": st["selected_long"],
                "test_short": st["selected_short"],
            })

        print("ORIENTATION " + " ".join(f"{r}={'N' if orientations[r] == 1 else 'I'}" for r in RULES))
        agg = {k: [0, 0] for k in ("LONG", "SHORT", "HOLD", "ALL")}
        for symbol, candles in [(s, resample(load_binance(find_month_file(root, s, args.test_month)), tf)) for s in symbols if find_month_file(root, s, args.test_month)]:
            rows, st = vote_dataset(candles, orientations, args.min_active)
            for row in rows:
                row["symbol"] = symbol
                row["timeframe_min"] = tf
            all_bar_rows.extend(rows)
            agg["LONG"][0] += st["long_n"]; agg["LONG"][1] += st["long_correct"]
            agg["SHORT"][0] += st["short_n"]; agg["SHORT"][1] += st["short_correct"]
            agg["HOLD"][0] += st["hold_n"]
            agg["ALL"][0] += st["all_n"]; agg["ALL"][1] += st["all_correct"]

        print("VOTE LONG_N LONG_ACC SHORT_N SHORT_ACC HOLD ALL_N ALL_ACC")
        print(f"VOTE {agg['LONG'][0]:6} {pct(agg['LONG'][1],agg['LONG'][0]):8.2f} {agg['SHORT'][0]:7} {pct(agg['SHORT'][1],agg['SHORT'][0]):9.2f} {agg['HOLD'][0]:5} {agg['ALL'][0]:5} {pct(agg['ALL'][1],agg['ALL'][0]):7.2f}")

    rp = Path(args.rule_output); rp.parent.mkdir(parents=True, exist_ok=True)
    with rp.open("w", newline="", encoding="utf-8") as f:
        fields = list(rule_rows[0].keys()) if rule_rows else ["timeframe_min", "rule"]
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader(); w.writerows(rule_rows)

    p = Path(args.output); p.parent.mkdir(parents=True, exist_ok=True)
    with p.open("w", newline="", encoding="utf-8") as f:
        fields = list(all_bar_rows[0].keys()) if all_bar_rows else ["symbol", "timeframe_min", "bar_index", "decision"]
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader(); w.writerows(all_bar_rows)

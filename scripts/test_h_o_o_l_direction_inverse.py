...
def load(path: Path) -> pd.DataFrame:
    ...
    # Inverse of the direct magnitude-side rule tested previously.
    # Larger H-O magnitude predicts SHORT; larger O-L magnitude predicts LONG.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Test the inverse H-O/O-L magnitude rule as a direction classifier")
    ...
    print("=" * 92)
    print("H-O / O-L MAGNITUDE RULES — INVERSE DIRECTION TEST")
    print("=" * 92)
    print("LONG rule magnitude:  C + |H-O|")
    print("SHORT rule magnitude: C - |O-L|")
    print("inverse direction: LONG when |O-L| > |H-O|, SHORT when |H-O| > |O-L|")

...
def predict_next(o: float, h: float, l: float, c: float) -> tuple[float, float, float, float, int]:
    """Build one synthetic next candle from the supplied candle only."""
    ...
    ph = max(po, pc, po + upper_ratio * magnitude)
    pl = min(po, pc, po - lower_ratio * magnitude)

    _pred_score, pred_direction = score_direction(po, ph, pl, pc)
    return po, ph, pl, pc, pred_direction


def three_frame_prediction(df: pd.DataFrame, i: int) -> tuple[int, int, int, bool]:
    """Predict t+1, t+2 and t+3 recursively from the preceding prediction."""
    ...
    return p1[4], p2[4], p3[4], p1[4] == p2[4] == p3[4]
...
def evaluate_symbol(
    ...
    eligible = agreement = trades = wins = 0
    gross_sum = 0.0
    ...
    while i < n - 3:
        ...
        eligible += 1
        p1_dir, p2_dir, p3_dir, same = three_frame_prediction(df, i)
        if same:
            agreement += 1
        if not same or p1_dir != entry_dir or p2_dir != entry_dir or p3_dir != entry_dir:
            i += 1
            continue
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="H3 recursive prediction agreement test: require t+1, t+2 and t+3 predictions to match current direction."
    )

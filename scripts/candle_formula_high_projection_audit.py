...
def main():
    ...
    a=ap.parse_args()
    print('HIGH_PROJECTION_AUDIT | tf=1h | fee=0 | exact rule: prediction = current close + (current high-current close) = current high')
    print('Compare predicted price with NEXT candle close. Also report direction, absolute error, percentage error, and zero-fee one-bar edge.')
    for sym in SYMBOLS:

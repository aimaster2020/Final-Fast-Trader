...
def backtest(pt_rows, max_duration, commission):
    ...
    def close_trade(row, bar, reason):
        ...
        bars = bar - entry_bar
        completed.append({"direction": direction, "gross": gross, "net": net, "win": net > 0, "bars": bars, "reason": reason})
        exits += 1
    ...
    for bar, row in enumerate(pt_rows):
        ...
        # Time-limit exit has priority at exactly max_duration bars held.
    ...
    if position != 0 and entry_price is not None:
        ...
        if position == 1:
            ...
            direction = "SHORT"
        open_trade = {"direction": direction, "gross": unrealized, "net_after_entry_fee": unrealized - commission, "bars": len(pt_rows) - 1 - entry_bar}

    return completed, open_trade, {"entries": entries, "exits": exits, "long_entries": long_entries, "short_entries": short_entries, "long_exits": long_exits, "short_exits": short_exits, "forced": forced}
...
def main():
    ...
    fields = ["trade_id", "direction", "bars", "gross_return", "net_return", "win", "reason"]

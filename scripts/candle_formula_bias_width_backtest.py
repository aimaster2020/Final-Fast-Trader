...
BIAS_WIDTHS = (1.0, 5.0, 10.0, 25.0, 50.0, 100.0)
CAPITAL = 1000.0
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sweep zero-centered bias widths")
    ...
    print(
        f"BIAS_WIDTH_SWEEP | tf=1h | main=S3,2_BUY/S0,1_SELL | "
    ...
    for symbol in SYMBOLS:
        ...
        for width in BIAS_WIDTHS:
            ...
            if best is None or (comp, avg, worst, -total_trades) > best[1:]:
                best = (width, comp, avg, worst, total_trades)
            print(
        ...
        if best:
            width, comp, avg, worst, total_trades = best
            print(
                f"BEST width={width:.0f} range=[{-width:.0f},{width:.0f}] "
                f"4M={comp:+.2f}% avg={avg:+.2f}% worst={worst:+.2f}% trades={total_trades}"
            )

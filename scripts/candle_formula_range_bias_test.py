...
def run_backtest(
    ...
    for candle in candles:
        ...
        if position is None:
            # Asymmetric entry range: inside [lower, upper] = no entry.
            ...
            if opposite:
                # Direction-specific exit bias:
                # LONG exits when BODY <= upper threshold.
                # SHORT exits when BODY >= lower threshold.
                exit_triggered = (
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Comma-separated candidate thresholds; each lower<upper pair is tested.",
    )
    ...
    for timeframe, path in jobs:
        ...
        results.sort(
            key=lambda x: (float(x[2]["return_pct"]), float(x[2]["profit_factor"])),
            reverse=True,
        )
        ...
        if baseline:
            _, _, r = baseline
            print(
                f"BASELINE[-100,+100] {timeframe} | return={float(r['return_pct']):.2f}% trades={int(r['trades'])} win={float(r['win_rate']):.2f}%"
            )

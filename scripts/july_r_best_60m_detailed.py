...
def chain_detail(cs: list[Candle], i: int, c: dict) -> tuple[bool, int, list[dict]]:
    ...
    steps = [{
        ...
        "variant": c["variant"],
        "mode": "TRIGGER",
        "raw_signal": raw_sig(cs[i], c["trigger"]),
        "signal": trigger,
        "expected": trigger,
        "ok": bool(trigger),
    ...
    for k, x in enumerate(c["selected"], 1):
        j = i + k
        if j >= len(cs):
            steps.append({"step": k, "rule": x["rule"], "variant": x["variant"], "mode": x["mode"], "raw_signal": 0, "signal": 0, "expected": trigger if x["mode"] == "SAME" else -trigger, "ok": False, "timestamp": ""})
        ...
        steps.append({
            ...
            "signal": s,
            "expected": expected,
            "ok": bool(s and s == expected),
            "timestamp": cs[j].timestamp,
        })
...
def run(series: dict[str, list[Candle]], fee_side_pct: float, trace_dir: Path) -> dict:
    ...
    for symbol, raw in series.items():
        ...
        c15 = resample(raw, 15)
        c60 = resample(raw, 60)
        signals, signal_rows = signal_times_15m_detailed(c15, {"trigger": "R16", "variant": "I", "selected": [{"rule": "R8", "variant": "I", "mode": "OPPOSITE"}, {"rule": "R6", "variant": "I", "mode": "OPPOSITE"}]})
        for r in signal_rows:
            r["symbol"] = symbol
        all_15_rows.extend(signal_rows)

        signal_by_id = {sid: (end, direction) for end, direction, sid in signals}
        signal_ptr = 0
        active_trade = None
        symbol_rows = []
        j = 0
        while j < len(c60):
            ...
            direction = 0
            signal_end = ""
            # Match the production/research rule: first 60m candle timestamp >= signal_end.
        ...
        all_60_rows.extend(symbol_rows)
        total += local
    ...
    write_csv(trace_dir / "best60m_15m_decisions.csv", all_15_rows)
    write_csv(trace_dir / "best60m_60m_frames.csv", all_60_rows)
    write_csv(trace_dir / "best60m_trades.csv", all_trade_rows)
...
def main() -> None:
    ...
    print(f"BEST_60M_DETAIL candidate={BEST} architecture=B assets={len(series)} test={args.test_month} fee={args.fee_side_pct:.3f}%/side")
    print("15m: R16 inverted -> R8 inverted OPPOSITE -> R6 inverted OPPOSITE")
    print("60m: first candle >= confirmed signal_end is entry; next 60m candle is exit")
    print("---")
    ...
    print("SAVED reports/july_r_best_60m_detail/best60m_15m_decisions.csv")
    print("SAVED reports/july_r_best_60m_detail/best60m_60m_frames.csv")
    print("SAVED reports/july_r_best_60m_detail/best60m_trades.csv")

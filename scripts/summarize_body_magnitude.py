...
def regression(df: pd.DataFrame, features: list[str], target: str = "U") -> dict[str, float]:
    ...
    out = {"n": float(len(data)), "r2": float(r2), "mae": float(np.mean(np.abs(err))), "rmse": float(np.sqrt(np.mean(err ** 2))), "corr": float(np.corrcoef(pred, y)[0, 1]), "intercept": float(beta[0])}
...
def main() -> None:
    ...
    lines = ["BODY MAGNITUDE SUMMARY", f"month={args.month}", f"rows={len(df)}", f"symbols={','.join(sorted(df['symbol'].dropna().unique()))}", ""]
    lines += summarize(df, "ALL")
    for symbol in SYMBOLS:
        lines += summarize(df[df["symbol"] == symbol].copy(), symbol)

...
def frame_prediction(candles: list[Candle], i: int, rule: str, inverted: bool, mode: str) -> int:
    ...
    # Four latest completed 1h candles: current candle + previous 3 candles.
    start = max(0, i - 3)
    votes = [signal(candles[j], rule, inverted) for j in range(start, i + 1)]
...
def backtest_symbol(candles: list[Candle], rule: str, inverted: bool, mode: str, fee_side_pct: float, horizon: int = 4) -> dict:
    ...
    peak = capital
    max_dd = 0.0
    start_i = 3 if mode == "4FRAME" else 0

    for i in range(start_i, len(candles) - horizon):
        if position:
            if i < exit_i:
                continue
            # Position is exited at the 4h horizon close.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="R1-R16 and inverses: 1h signal with 4h horizon, current candle vs 4 latest 1h-frame vote.")
    ...
    ap.add_argument("--output", default="reports/july_r1_r16_1h_horizon4_compare.csv")
    args = ap.parse_args()
    ...
    print(f"JULY_R1_R16_1H_H4 test={args.test_month} assets={len(series)} initial={args.initial_capital:.2f} fee={args.fee_side_pct:.3f}%/side")
    print("HORIZON=4h | SIGNAL=1h | CURRENT=current 1h candle | 4FRAME=current + previous 3 completed 1h candles (majority vote)")
    print("NO indicators | NO ML | R1-R16 frozen definitions | N=normal, I=inverse | non-overlap trades, one position per symbol")
    ...
    for rule in RULES:
        for inverted in (False, True):
            ...
            for mode in MODES:
                ...
                # backtest_symbol currently uses 250 per asset; rescale only if custom initial capital is requested.
                ...
                rows.append({
                    "rule": rule, "mode": mode, "variant": variant,
                    "initial_capital": args.initial_capital, "final_capital": total_final,
                    "return_pct": ret, "directional_accuracy_pct": acc,
                    "trades": trades, "win_rate_pct": win, "max_dd_pct": dd,
                    "fees_paid": fees, "test_month": args.test_month,

...
def load_month(path: Path, month: str, symbol: str) -> list[Candle]:
    ...
    with path.open("r", newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            if row.get("month") != month or (symbol and row.get("symbol") != symbol):
                continue
            try:
                candles.append(
                    Candle(
...
def next_direction(current: Candle, following: Candle) -> str:
    change = following.close - current.close
    if change > 0:
        return "UP"
    if change < 0:
        return "DOWN"
...
def run_timeframe(path: Path, timeframe: str, month: str, symbol: str, output: Path) -> dict[str, object]:
    ...
    fields = [
        ...
        "next_close", "actual_next_direction", "up_rule_1", "up_rule_2", "up_rule_3",
    ...
    with output.open("w", newline="", encoding="utf-8") as f:
        ...
        for i in range(len(candles) - 1):
            ...
            rules = evaluate_rules(candle)
            actual = next_direction(candle, following)
            ...
            if predicted in {"UP", "DOWN"} and actual in {"UP", "DOWN"}:
                ...
                if predicted == "UP" and actual == "UP":
                    ...
                    du += 1
                elif predicted == "DOWN" and actual == "DOWN":
                    dd += 1
    ...
    up_acc = uu / predicted_up * 100.0 if predicted_up else 0.0
    down_acc = dd / predicted_down * 100.0 if predicted_down else 0.0

    return {
        "timeframe": timeframe, "candles": total,
        "actual_up": actual_up, "actual_down": actual_down, "actual_flat": actual_flat,
        "up": predicted_up, "down": predicted_down, "undecided": undecided,
        "checked": comparable, "wins": wins, "accuracy": accuracy, "coverage": coverage,
        "range": ranges, "range_pct": range_pct, "up_acc": up_acc, "down_acc": down_acc,
        "uu": uu, "ud": ud, "du": du, "dd": dd, "stats": stats, "output": str(output),
    }


def main() -> None:
    ap = argparse.ArgumentParser(description="Leakage-safe next-candle validation of candle_formula_weighted_v2.")
    ...
    ap.add_argument("--output-dir", default="reports/candle_formula_next_candle_validation")
    args = ap.parse_args()
    ...
    for timeframe, path in jobs:
        ...
        print(f"{timeframe} {args.month} | N->N+1 candles={result['candles']} | ACTUAL U={result['actual_up']} D={result['actual_down']} F={result['actual_flat']}")
        ...
        print(f"  WEIGHTED U={result['up']} ({result['up_acc']:.2f}%) | D={result['down']} ({result['down_acc']:.2f}%) | UNDECIDED={result['undecided']} | RANGE={result['range']} ({result['range_pct']:.2f}%)")
        print(f"  CHECKED={result['checked']} WINS={result['wins']} ACC={result['accuracy']:.2f}% COVERAGE={result['coverage']:.2f}%")
        print(f"  CONFUSION UU={result['uu']} UD={result['ud']} DU={result['du']} DD={result['dd']}")
        print(f"  CSV={result['output']}")

    summary = Path(args.output_dir) / f"summary_{args.symbol}_{args.month}.csv"
    summary.parent.mkdir(parents=True, exist_ok=True)
    with summary.open("w", newline="", encoding="utf-8") as f:
        fields = ["symbol", "month", "timeframe", "candles", "actual_up", "actual_down", "actual_flat", "predicted_up", "predicted_down", "undecided", "checked", "wins", "accuracy_pct", "coverage_pct", "range", "range_pct", "up_accuracy_pct", "down_accuracy_pct", "uu", "ud", "du", "dd"]
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for r in results:
            writer.writerow({
                "symbol": args.symbol, "month": args.month, "timeframe": r["timeframe"], "candles": r["candles"],
                "actual_up": r["actual_up"], "actual_down": r["actual_down"], "actual_flat": r["actual_flat"],
                "predicted_up": r["up"], "predicted_down": r["down"], "undecided": r["undecided"],
                "checked": r["checked"], "wins": r["wins"], "accuracy_pct": f"{r['accuracy']:.6f}",
                "coverage_pct": f"{r['coverage']:.6f}", "range": r["range"], "range_pct": f"{r['range_pct']:.6f}",
                "up_accuracy_pct": f"{r['up_acc']:.6f}", "down_accuracy_pct": f"{r['down_acc']:.6f}",
                "uu": r["uu"], "ud": r["ud"], "du": r["du"], "dd": r["dd"],
            })
